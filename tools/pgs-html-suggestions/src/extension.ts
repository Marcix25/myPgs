...
//+ shape of reference/pgs-map.json, generated by node scripts/generate-pgs-map.js: one entry per
//+ root pgs token, with the child tokens it declares, the ones the library generates at runtime,
//+ the CSS/JS-only flags in its own bracket, and the pgs-data keys it accepts
interface PgsMapEntry {
    pgs: string[];
    "pgs-generated": string[];
    "pgs-options": string[];
    "pgs-data": string[];
}

type PgsMap = Record<string, PgsMapEntry>;

//+ everything the extension needs to know about one root, precomputed once per load so
//+ completion never has to walk the whole map on every keystroke
interface RootInfo {
    name: string;
    options: string[];
    data: string[];
...
class PgsMapIndex {
    ...
    private getMapPath(): string {
        return this.getConfig().get<string>("mapPath", "reference/pgs-map.json");
    }
    ...
    resolveRoot(token: string): RootInfo | undefined {
        ...
        if (viaGenerated) {
            ...
            if (entry) return { name: viaGenerated, options: entry["pgs-options"] ?? [], data: entry["pgs-data"] ?? [] };
        }
    ...
    allBareTokens(): { token: string; kind: "root" | "child" | "generated" }[] {
        ...
        const push = (token: string, kind: "root" | "child" | "generated") => {
    ...
    allDataKeys(): string[] {
        ...
        for (const entry of Object.values(this.map)) {
            for (const key of entry["pgs-data"] ?? []) keys.add(key);
        }
    ...
    allOptionFlags(): string[] {
        const keys = new Set<string>();
        for (const entry of Object.values(this.map)) {
            for (const key of entry["pgs-options"] ?? []) keys.add(key);
        }
        return [...keys].sort((a, b) => a.localeCompare(b));
    }

    private setupWatcher(context: vscode.ExtensionContext): void {
        ...
        for (const folder of folders) {
            const watcher = vscode.workspace.createFileSystemWatcher(
                new vscode.RelativePattern(folder, mapPath)
            );
...
function dataKeyInsertText(key: string): vscode.SnippetString {
...
interface PgsAttributeState {
    //+ raw text of the pgs="..." value, from its opening quote up to the cursor
    valueSoFar: string;
    //+ set only when the cursor is inside an unclosed component[...]
    bracketOwner?: string;
    //+ the partial text already typed inside that bracket, used to avoid re-suggesting a flag
    //+ already picked in the same bracket
    bracketPartial?: string;
}

function getPgsAttributeState(document: vscode.TextDocument, position: vscode.Position): PgsAttributeState | undefined {
...
function tokenizeBracketAware(source: string): string[] {
    const tokens: string[] = [];
    let i = 0;
...
function flagsAlreadyInBracket(bracketPartial: string): Set<string> {
...
function currentTagTextBeforeCursor(document: vscode.TextDocument, position: vscode.Position): string {
...
function getSiblingPgsValue(document: vscode.TextDocument, position: vscode.Position): string | undefined {
    const tagText = currentTagTextBeforeCursor(document, position);
    const match = /\bpgs\s*=\s*"([^"]*)"/.exec(tagText);
    return match?.[1];
}

function rootNamesFromPgsValue(index: PgsMapIndex, pgsValue: string): RootInfo[] {
...
//+ JS-side context: pgs(el).option.METHOD("...") or pgs(el).data.METHOD("...") — there is no
//+ element to read a sibling attribute from here, so the suggestions fall back to every flag or
//+ every data key in the project
type JsCallKind = "option" | "data" | undefined;

const JS_CALL_OPEN = /\.\s*(option|data)\s*\.\s*(?:contains|add|remove|toggle|getValueBrackets|setValueBrackets)\s*\(\s*(["'`])[^"'`]*$/;

function getJsCallKind(document: vscode.TextDocument, position: vscode.Position): JsCallKind {
    const match = JS_CALL_OPEN.exec(linePrefix(document, position));
    return match ? (match[1] as "option" | "data") : undefined;
}

function makeFlagCompletion(flag: string, detail: string, alreadyPicked: boolean): vscode.CompletionItem {
...
function makeDataCompletion(key: string, detail: string): vscode.CompletionItem {
...
function makeBareTokenCompletion(token: string, kind: "root" | "child" | "generated", alreadyUsed: boolean): vscode.CompletionItem {
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("pgsHtmlSuggestions.rescan", async () => {
            await index.load();
            vscode.window.showInformationMessage("PGS: pgs-map.json ricaricato.");
        })
    );

    const markupProvider: vscode.CompletionItemProvider = {
        provideCompletionItems(document, position) {
            ...
            return index.allBareTokens().map(({ token, kind }) =>
                makeBareTokenCompletion(token, kind, alreadyUsed.has(token))
            );
        }
    };

    const jsProvider: vscode.CompletionItemProvider = {
        provideCompletionItems(document, position) {
