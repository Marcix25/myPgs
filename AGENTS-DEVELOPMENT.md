...
- `.option.add(key)` derives the owning component from `key`'s own name — the lowercase run before the first uppercase letter or a `-` — and merges into that component's existing bracket if a token with that key is present on the element. Most flags lost that derivable prefix when they were shortened, so this only actually resolves an owner for the few that kept the component's own full name (`icon-moon` → `icon`, `cardMini` → `card`); an abbreviated prefix does not count — `mrgLeft`/`padTop` derive `mrg`/`pad`, not `margin`/`padding`, and `btnMini` derives `btn`, not `button` — so those fall through the same as any other shortened flag, becoming their own bare `pgs` token, same as `hoverNot` (which never had an owner to derive). To add a shortened flag into its bracket, call the base `pgs(el).add("component['flag']")` directly, naming the component — this is now the normal way to add a bracket flag from JavaScript, not a fallback. `option.remove`/`option.toggle` strip a flag correctly either way, bare or nested, since removal only needs to find the flag, not derive where to put it. There is no ownership registry to keep in sync.
