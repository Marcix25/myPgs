...
- CSS flags and JavaScript-only boolean flags both live inside their component bracket, shortened to drop the component's own name from the flag now that the bracket already provides it: `pgs="button['transparent']"`, `pgs="header['headerScroll']"` (not `button['buttonTransparent']`, `header['headerScroll']`). A handful kept a prefix on purpose — the `icon-*` glyphs, margin/padding's own `mrg*`/`pad*` flags (folding the direction into the name instead of a separate root per direction), and a few single-component flags renamed to stay unique once another component needed the same short name (`button['btnMini']`, not `button['mini']`, since card and box also have a `mini`) — and `hoverNot` never had a single owner to drop a prefix from in the first place.
...
- `.option.add(key)` derives the owning component from `key`'s own name — the lowercase run before the first uppercase letter or a `-` — and merges into that component's existing bracket if a token with that key is present on the element. Most flags lost that derivable prefix when they were shortened, so this only actually resolves an owner for flags whose prefix is the component's own full name (`icon-moon` → `icon`, `cardMini` → `card`, `badgeDot` → `badge`, `menuVertical` → `menu`); an abbreviated prefix does not count — `mrgLeft`/`padTop` derive `mrg`/`pad`, not `margin`/`padding`, `btnMini` derives `btn`, not `button`, and the same goes for `acc*`, `drp*`, `sct*`, `shell*`, `tgl*` — so those fall through the same as any other shortened flag, becoming their own bare `pgs` token, same as `hoverNot` (which never had an owner to derive). To add a shortened flag into its bracket, call the base `pgs(el).add("component['flag']")` directly, naming the component — this is now the normal way to add a bracket flag from JavaScript, not a fallback. `option.remove`/`option.toggle` strip a flag correctly either way, bare or nested, since removal only needs to find the flag, not derive where to put it. There is no ownership registry to keep in sync.
...
- Every bracket flag carries a prefix of its own component, so two components on the same element can never match each other's flag (`[pgs*="'mini'"]` cannot tell button's from card's). A short component name is written in full (`cardMini`, `badgeDot`, `iconLarge`, `menuVertical`, `slidesSingleScroll`, `headerScroll`); a long one is abbreviated (`btn`, `acc`, `drp`, `sct`, `mrg`, `pad`, `brd`, `shell`, `tgl`, `rad`, `cnt`). Once a component has a prefix, every new flag of it uses the same one. flex/grid's layout flags (`column`, `row`, `gapTexts`, `itemCenter`, ...) are the one family still bare, and `hoverNot` stays bare because it has no single owner.
