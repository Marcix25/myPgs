...
`reference/html/` has five top-level folders — `base/`, `components/`, `layout/`, `patterns/`,
`helper/` — each becoming its own section in the demo's side menu, in the order that
`demoRenderer.referenceFiles` (see `demo.js`) lists them; a file's position within a category
follows that same array. `docs/` mirrors the exact same folder structure and filenames, just with
`.md` instead of `.html`.

## Demo markup: `demo`, `demo-title`, `demo-description`, `demo-code`, `demo-preview`

These attributes only mean something to the demo/doc tooling — strip them and what's left is the
real example a consumer would copy.

- **`demo="component"`** marks one independent example within a file that has several (see
  `modal.html`'s many `<div pgs="modal" demo="component" demo-title="..." demo-description="...">`
  blocks, one per variant). Pair it with `demo-title`/`demo-description` to caption it.
