...
//= pgs/pgs-option/pgs-state attribute usage, used to group @related entries by how they're used
...
function renderDocumentationHtml(data, markup, basename, cssText) {
    ...
    const parts = [
        ...
        renderDocGroupHtml(LIST_TAG_LABELS["pgs-option"], data["pgs-option"] || [], "h4"),
...
function renderHeadingBlockHtml(title, description, level) {
    ...
    return `<div class="demoContent-heading" pgs="flexColumn" pgs-option="gapTexts">${heading}${desc}</div>`;
}
...
function renderReferenceHtml(markup) {
    return `<div pgs="container flexColumn" pgs-option="gapElements">${markup}</div>`;
}

function renderDemoItemHtml({ previewMarkup, codeMarkup, title, description, showPreview, hasCode }) {
    let html = `<div class="demo-item" pgs="flexColumn" pgs-option="gapElements">`;
    html += renderHeadingBlockHtml(title, description, "h3");
    html += showPreview
        ? `<div pgs="container flexColumn" pgs-option="gapElements">${previewMarkup}</div>`
...
function renderNavMenuHtml(items, category) {
    const rows = items.map(({ path }) => {
        ...
        return `<li><a href="#${escapeHtml(getSlug(path))}" data-panel-link="${escapeHtml(path)}" pgs="button" pgs-option="buttonText buttonPaddingEqual">` +
...
function renderReferencePanelHtml(path, rawFileText, cssText) {
    ...
    try {
        ...
        if (jsonSchema) bodyHtml += renderExampleSourceHtml(`pgs-option='${jsonSchema}'`, "PGS Option fields", "json");
    ...
    const sectionExtraAttrs = isSection ? ` pgs="flexColumn" pgs-option="gapElements"` : ` style="display:contents"`;
    const section = `<${sectionTag} class="demoContent" data-reference="${escapeHtml(path)}"${sectionExtraAttrs}>${bodyHtml}</${sectionTag}>`;
    const panel = `<div data-panel="${escapeHtml(path)}" hidden pgs="flexColumn" pgs-option="gapElements">${section}</div>`;
