...
Toast markup, configuration, behavior, and usage example. An ephemeral, auto-dismissing message shown one at a time (e.g. "saved", "error"). Split from Notification, which is a persistent, manually-dismissed panel. Full JSON field list is under @pgs-data "toast"; full JS options list is under each @api method below. The JSON form and the JS options now use the same names; the previous message, title-link, title-close and duration keys are still accepted as aliases.

## PGS

- `toastLoad`: creates one toast from a JSON object on page load; see @pgs-data "toast" for every accepted field.
- `toastExe`: creates one toast from a JSON object each time the element is clicked; same fields as toastLoad.
- `toast`: write it yourself, bare like this, to place the container elsewhere; the module leaves it alone and never generates a second one. Leave it empty, because its content is replaced at every toast.

## PGS generated by JavaScript

- `_toast`: the container the toasts are shown in, appended at the end of the body when no toast was written by hand.
...
- `btnIconOnly`: shapes that dismiss button as an icon-only button; see Button.
...
<div pgs="hidden toastLoad" pgs-data='toast[{
        "title": "Benvenuto",
        "description": "Message",
        "type": "success",
        "icon": null,
...
<button pgs="button toastExe" pgs-data='toast[{
    "title": "Benvenuto",
    "description": "Message",
    "type": "success",
    "icon": null,
