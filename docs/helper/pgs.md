...
The `pgs(root)` function is how every component finds and edits its own markup: it wraps a Document or Element and returns read/write helpers for the pgs, pgs-option and pgs-state attributes, so nothing in the library — or in your own code — needs to hand-write an attribute selector. querySelector/querySelectorAll work on a Document too; every other method needs an Element and throws otherwise.
...
- `instance.contains(token)`: checks whether the exact pgs token is present.
