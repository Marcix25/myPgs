...
<p pgs="box widthMax marginInline['auto']" style="--width-size: 500px">Capped at five hundred pixels and centred.</p>
```
...
auto removes the limit on a single element while the utility stays in place, so a rule that sets --height-size higher up is not fought with a second one.
